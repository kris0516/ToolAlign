...
P00已冻结、合并并验证；P02数据代码已合并且main技术验证通过。P01独立R1-r2正式FAIL，仅剩启动初始化P1，T1空闲、修复范围已准备；P03最终3598cef已交付/原生空闲，R1-r2范围已准备。R1当前P01轮次的原生终态须核验后才接续，D1仍在CPU提案范围内比较格式，不切生产默认。后续派发仍最多两个并行实现加独立CPU R1。P02/G-DATA仍待kris实际语义审查和训练绑定，P04尚未授权。不重问已确定的硬件/目标/独立对话约束；超出明确资源/费用/发布边界的动作单独交kris决定。
