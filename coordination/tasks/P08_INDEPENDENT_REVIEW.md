...
|---|---|
| owner 角色 | R1 |
| 依赖 | 分阶段评审；最终依赖P06/P07及对应release范围 |
| base commit | UNASSIGNED — S0 分发前填实际完整 SHA |
| branch/worktree | UNASSIGNED — 每包独立，不共用 checkout |
| 契约 | toolalign.*.v1，P00 冻结后填写精确版本 |
| 交接 | `coordination/handoffs/P08-r1.md` |

## 目标

独立检查代码差异、原始证据、数据隔离、DPO语义、oracle、安全、资源锁和发布内容。

## 允许修改范围

reports/reviews、独立复现说明；若新增审查测试先和S0确认专用路径。

除此之外文件默认只读；公共契约、依赖锁和main合并权归S0。路径尚未创建时，先核对P00结构，不能各自发明一套。

## 非目标

不偷偷修实现再自签PASS，不要求非阻断建议全部完成才认可有效交付，不假称跑过未拥有的硬件。

## 验收

逐项PASS/FAIL/BLOCKED；实际命令与exit code；至少一个负向复现；精确被审commit；对数据/指标/ref/cache/任意执行的明确结论。
...
证据缺失先BLOCKED而非猜测；P0返工到原worker；只对核验范围给结论。
