...
4. 只调用固定上游 `trainer.train` 的公开参数，注入已审 completion loss 与有限 iterator；不复制循环或改 vendor，保留原生 `mx.compile` 并固定 `grad_checkpoint=False`，不自动切换两项设置。固定 Adam(lr=1e-4, betas=[0.9,0.999], eps=1e-8, bias_correction=False)，无 weight decay/scheduler；microbatch1、seed42、原顺序、无 packing/shuffle/截断。每例完成 token mean 后按本累积组真实例数平均梯度，validation 用总 CE/总监督 token。
...
7. 每段 `val_dataset=None`，局部 save/eval 周期设为本段微步+1，report周期为divisor；上游返回保存仍逐一核对。三次完整 post-update validation：smoke steps66/132/198，formal250/500/743，尾段前文件未评分。每次 validation 使用新有限 iterator，finally 恢复原 training 模式，核对前后模型/optimizer/RNG/step/文件不变。容量的前8例过拟合CE属于原train，只作固定诊断，不改写split或作为validation分数；旧 `ValidationTotals` 的validation约束不放宽。中途异常或元数据缺失不产生可选 score。
