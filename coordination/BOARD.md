...
| [P01](tasks/P01_HARDWARE_COMPATIBILITY.md) 本机校准与兼容 | T1 | P00 | IN_PROGRESS | 原候选`f97bb0d`保留；已派CPU基线同步（生产37c00de/授权f2a271b），待新完整候选与R1 |
| [P02](tasks/P02_DATA_PIPELINE.md) 数据治理 | D1 | P00 | IN_PROGRESS | 原候选`46f5465`保留；已派CPU基线同步（生产37c00de/授权f2a271b），8,228决策/两遍证据及未填人审保持 |
...
公共跟进：[S0-SHARED-02 / PR4](https://github.com/kris0516/ToolAlign/pull/4) 两轮嵌套/大小写P1均由R1-r3独立关闭。[已验证生产base](../reports/S0_SHARED_02_MAIN_VERIFICATION.md)为`37c00de`。D1/T1已收到授权`f2a271b`并原生核验活跃，均仅CPU同步；E1保持空闲待下一批，当前正好两个实现任务。旧基线仍禁止默认源码包构建/发行。
