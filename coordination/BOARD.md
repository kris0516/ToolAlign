...
| [P04-SFT-CPU](tasks/P04_SFT_CPU_PREPARATION.md) SFT数据/collator与数值衔接 | T1；R1独立审查 | 技术基线42eaa50、ADR-0020 | READY_FOR_REVIEW（CPU部分）；R1 IN_PROGRESS | 完整33d6248已交付/T1空闲，S0核对2083路径及18原命令/三归档/安装；970/2是T1自测，原生train仍BLOCKED。[R1](tasks/P04_SFT_CPU_REVIEW.md)按完整d65592e实际派发/ACTIVE，384候选/13授权intake已核验；[Draft PR10](https://github.com/kris0516/ToolAlign/pull/10)候选双Python CI通过/390文件绑定，见[证据](../reports/S0_P04_SFT_CPU_HANDOFF.md) |
...
公共跟进：PR8共用格式/截止时间修订及PR9训练绑定CPU均VERIFIED。T1的P04-SFT-CPU完整33d候选已交付并原生结束；D1/E1/T1空闲。R1按完整d65592e审精确CPU部分候选，新原生轮ACTIVE；PR10保持Draft，候选双Python CI已通过并核对实际合并源码。已知原生训练入口阻塞保留，数值复核须持共享租约并强制CPU；语义/实际页面/token-mask人工与正式P04仍待完成。
