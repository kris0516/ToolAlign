...
实际核验失败保留两项：E1 材料检查器初版误套旧 HTML 独立 ModelInput 区块，改为按实际完整 Example 比对后 exit0；最终清单脚本首次因先前 fixture 留下的本目录符号链接中止，改为记录链接本身且不跟随，最终封存结果随原生交接登记。两次原 exit1 日志及失败脚本字节均保留，没有修改输入材料、判断或删除其他断言。辅助只读路径定位失败及显示截断的完整补读另有记录；不伪造未落盘的原始日志。
