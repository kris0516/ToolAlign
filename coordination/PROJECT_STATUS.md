...
更新时间：2026-09-06。当前交付状态：**P00_VERIFIED**，两个共享支持包已VERIFIED。P01/P02/P03均保留已交付候选；D1/T1现已派发CPU基线同步，E1空闲待名额；各包等待独立审查，P02另需kris实际数据语义审查。
...
| 独立实现/reviewer 对话 | T1/P01、D1/P02已原生派发并核验活跃，仅CPU基线同步；E1/P03空闲待名额；R1已完成共享审查并空闲 |
...
P00自测、R1精确head独立审查、S0合并与main重验均已满足。第一批P01/T1与P02/D1之后，T1交付且原生空闲才派P03/E1（base97466a2/授权e882594）。三个原候选现均已正式交付；S0发布共享base37c00de和授权f2a271b后，已原生派发并核验D1/T1两项CPU同步。E1继续空闲；后续仍最多两个并行实现，不从文档推定运行状态。
