...
def installed(args):
    ...
    from toolalign.data import quality_exclusion as revision
    from toolalign.data import quality_exclusion_materials as materials
    ...
    config = revision.load_config(expected["config_path"])
    assert config["training_authorized"] is False
    bad = Path(args.output).parent / "altered-policy.json"
    with bad.open("xb") as stream:
        stream.write(Path(expected["config_path"]).read_bytes() + b"\n")
    try:
        revision.load_config(bad)
    except DataError as exc:
        assert str(exc) == "input_hash_mismatch"
    else:
        raise AssertionError("installed_consumer_accepted_replaced_policy")
    comparison = materials.compare(**expected["paths"], revision=expected["revision"], frozen=expected["frozen"], reference=expected["reference"], native=expected["native"], release_path=expected["release_path"])
    assert comparison["unique_examples"] == 13 and comparison["actual_new_sequence_calls_total"] == 4 and comparison["semantic_verdicts_filled"] == 0
    ...
    result = {"status": "PASS", "actual_Qwen_encoding": "NOT_RUN_IN_DEFAULT_INSTALL",
        "complete_material_comparison": comparison, "new_v3_output_builds": 0,
...
def archives_and_install(args):
    ...
    expected = {"package_files": {n: sha((root / "src" / n).read_bytes()) for n in package_names},
        "config_path": inputs["config_path"], "paths": inputs, "revision": str(Path(args.revision).resolve()), "frozen": str(Path(args.frozen).resolve()), "reference": str(Path(args.reference).resolve()),
        "native": str(Path(args.native).resolve()), "release_path": str(Path(args.release_path).resolve())}
    ...
    def run(label, command, expected_exit=0):
        ...
        record = {"label": label, "argv": command, "started_at_utc": started,
            "finished_at_utc": datetime.now(timezone.utc).isoformat(), "exit_code": done.returncode, "expected_exit_code": expected_exit,
            "stdout_sha256": sha(done.stdout), "stderr_sha256": sha(done.stderr)}
        save(out / (label + ".command.json"), record)
        commands.append(record)
        assert done.returncode == expected_exit, label
    ...
    bootstrap = "import sys; sys.path[:0]=[sys.argv.pop(1),sys.argv.pop(1)]; from toolalign.data.quality_exclusion import main; raise SystemExit(main())"
    ...
    run("installed-existing-output-rejected", base + ["build", *flags, "--output", str(Path(args.revision).resolve())], expected_exit=1)
    rejected = json.loads((out / "installed-existing-output-rejected.stdout").read_bytes())
    assert rejected["error_code"] == "output_already_exists"
    material_bootstrap = bootstrap.replace("quality_exclusion import", "quality_exclusion_materials import")
    run("installed-material-help", [sys.executable, "-B", "-I", "-S", "-c", material_bootstrap, str(target), str(runtime), "--help"])
    run("installed-verify-source-revision", base + ["verify", *flags, "--output", str(Path(args.revision).resolve())])
    summary = {"status": "PASS", "archives_sha256": sha((out / "archives.json").read_bytes()),
        ...
        "default_wheel_installed": True, "source_cwd_imports": 0, "new_v3_output_builds": 0}
...
def main():
    ...
    for key in ("root", "archives", "output", "runtime", "inputs", "revision", "frozen", "reference", "native", "release-path"):
