...
def installed(args):
    ...
    from toolalign.data import quality_revision as revision
    ...
    comparison = materials.compare(reference=expected["reference"], native=expected["native"])
    assert comparison["unique_examples"] == 16 and comparison["semantic_verdicts_filled"] == 0
...
def archives_and_install(args):
    ...
    expected = {"package_files": {n: sha((root / "src" / n).read_bytes()) for n in package_names},
        "config_path": inputs["config_path"], "reference": str(Path(args.reference).resolve()),
    ...
    bootstrap = "import sys; sys.path[:0]=[sys.argv.pop(1),sys.argv.pop(1)]; from toolalign.data.quality_revision import main; raise SystemExit(main())"
    ...
    run("installed-verify-quality-revision", base + ["verify", *flags, "--output", str(Path(args.revision).resolve())])
...
def main():
    ...
    for key in ("root", "archives", "output", "runtime", "inputs", "revision", "reference", "native"):
