...
2026-09-06，D1，`work/p02-data`，gpt-6-astra / max。状态：**READY_FOR_REVIEW**。最终被测完整候选为 `6c82d29ddaade349d3e2a15f50d35214dbc7bf8d`，包含修复提交 `b4dc1cf5c7d8c551506a7015bca2400573e16857` 及只读原审查 `2942e568eae91d0292ad9691af133bbd8c33dd02`。本报告、[证据索引](P02_FORMAT_FIX_R3_EVIDENCE.json)和[交接单](../../coordination/handoffs/P02-format-fix-r3.md)只增加证据；最终交接提交 SHA 由原生消息交给 S0，其余代码/测试/包载荷保持该完整候选字节。

真实 HF 路径已改为从已核验内存字节建立私有快照，再实际调用 `AutoTokenizer.from_pretrained`。同一个文件时序反例在原候选出现声明身份相同而 `encode("!")` 从 `[0]` 变成 `[30]`；修复后为 `[0]`，完整 backend 状态与正常基线相同。原始失败、native 对照及全部开发失败均保留。合并后适用回归实际分两组执行：**675 passed / 2 skipped** 和原 R1 **60 passed / 0 skipped**，两组不重叠，共 735 passed / 2 skipped；真实 HF 的本轮新测试另外 **13 passed / 0 skipped**。同 12 个 fixture 的两套真实 engine 输出与修复前逐项一致，当前三个归档和 10 条默认 CPU 安装命令通过。这是 D1 修复自测，尚待独立 R1 复审和 S0 验收。

**授权和范围。** 本轮完整授权 `c6c02a5af084afe92c6e9f05d9d1c51392e805d0`；契约为 `plan-v0.1` / `coordination.v1` / `toolalign.contracts.v1`，格式仍按 ADR-0017 和 [docs16](../../docs/16_MODEL_IO_FORMAT.md)。从干净 7bada2e 继续，只改 [offline.py](../../src/toolalign/model_io/offline.py)并新增 [test_snapshot.py](../../tests/model_io/test_snapshot.py)。既有 `test_offline.py` 未改，其他格式/序列/资源/fixtures、数据、P03、旧审查、规范、依赖与状态只读。S0 同轮授权 `6272ad512b947b9cf14c5f7201b7807f7f188e13` 后，先保存合并前证据为 `7b8ef310db214845ef5ac1fabfef1e37e9e05ab0`，再普通 merge 唯一公开 review `2942e568eae91d0292ad9691af133bbd8c33dd02` 得到 6c82d29，两个父提交分别为 7b8ef31 和 2942e56。12 份原 R1 文件逐字节匹配 S0 证明；正式结论仍为原候选 **FAIL / P0=0 / P1=0 / P2=1**，本轮只修 F1/P2。R1 本地原 `f7086413a9fedd9e2a473ac6d2869efff74ddad5` 与去敏公开 2942 是不同 SHA，原 f708 不在本候选祖先；未接入随后 main/P01，未改写原 FAIL。
...
原 native 8,228 行审计继续绑定 `b33a55fba61d39f9da3c224ae6743c7cf6328cf6`、原 loader 6d1a447、输入/hash/运行时间；原私有 manifest SHA `07daedb35169d6fa4d6eae1c7663387380035971c15c88ae5461ff4e7f617d23`，rows SHA `36b8cbfe6773c08f7a28521a99ed8783723a8f7fa3b2a3fa87915d8d1d871aff`。原公开序列 manifest SHA `69bfa651bf8db9b2c77c11c4f8d55419a8af4196aba8ff6a69b5b3b0982e0f47` 未改。新全量测量次数为 0；原 R1 公开 2942 的全量结果和 S0 证明已只读接入。R1 原 reference 运行仍绑定 7bada2e、原 loader 6d1a447、原环境和 2026-09-06 07:10:35–07:10:57 UTC；原日志 SHA `63c2fc41b766e4ae939d0062a1b9c65634e9a492694a7a111d13891fb1fdfe18`，result SHA `22d9cb07fcee18c36d94da73c0160e399b83a6d4ad452106acc241263f4e5ce8`，rows SHA 与上述原 native 相同。D1 本轮实际读取未改的正式公开结果，核对其中 rows hash 与保留的 native 行文件、所有 all/by_split 汇总完全相同；未直接重测 R1 全量或将其写成修复后执行。S0 对原材料的独立核验 proof SHA `5ddfdd75918be89160cc3e3cc2a680ad4ebba9cb94be3ae0b0932393a171450d` 已验证。R1 的 fsspec 为 2025.3.0，D1 既有 reference 为 2026.7.0，各自原环境身份分别保留。

**组合 CPU 与当前包。** 在 b4dc1cf 上使用既有组合命令运行全部 tests 以及未修改的 P00、P00-r2、P02、P03 lifecycle/semantics 独立测试：675 passed / 2 skipped，45.17 秒，日志 SHA `f5f4d626956e4e55ca0edf8c692616dd6546b6e55c70a83f4fff81fe5777f287`。其中既有 664 项加新增 native 11 项，两个 skip 仅为 HF 专属异常清理。合并后在完整 6c82d29 重新实际执行两组：原组合 675 passed / 2 skipped（44.82 秒），未改原 R1 结构/统计检查 60 passed / 0 skipped（0.11 秒）；总计 735 passed / 2 skipped，组合日志 SHA `9231f5162e4a06e321360aca16a118141051fe7106fc355a88cc44796187924d`。reference 新测试在同一完整提交另外 13 passed / 0 skipped（8.01 秒），日志 SHA `277ec149846f16f137938d103fae9628fec45cf94d8fcbbfd22ff24ab3ee6faa`，覆盖两个 HF 专属清理场景；不将重叠 native/reference 或安装重复相加。

合并后第一次尝试在同一 pytest 调用收集全部原文件，因 P00 与 P02-format 都有 `test_independent.py` 而 exit 2 / 1 collection error，日志 SHA `84b483292697fe79d6e865e0541680c02f0e4d2b8328c46f3b35073a00bab0ac` 与当时私有启动器保留。仅将私有启动器改为上述两组独立调用，保留 `__main__` 多进程入口保护；没有改测试名、原测试字节或全局收集规则。完整 ruff、四份冻结契约和公开扫描均通过。
...
默认 `uv build --offline --no-python-downloads` 实际从同一 sdist 构建 wheel；另显式重建。源码直接 wheel 为 **NOT_RUN**。沿用未修改的原包检查器，在本轮新 target 只安装默认 CPU 依赖，使用既有 Python 的 `-I -S` 从系统临时 cwd 检查；39 个包源码/资源与当前 Git 字节相符，12 个 fixture 的投影/编码/序列/parser 接口及五类契约验证通过。10 条安装/接口命令均 exit 0，实际安装仅六个默认包，可选 tokenizer/模型包不可导入。序列回调在此是透明字符测试替身；真实 Qwen 检查已在上述两个 CPU 环境单列。package summary SHA `e542e0195407bdf85540cb8b38ff8dfeebb97fc286b00e94c20507a3569e8f12`。构建和这 10 条纯默认接口命令实际发生于 b4dc1cf。合并只增加 reports/coordination，当前 sdist 的 87 个 Git 载荷文件和两个 wheel 各 39 个源码/资源仍逐字节相同；按 S0 授权关联现存三个制品，没有伪写新构建时间或改原运行 SHA。

在完整 6c82d29 **另把实际默认 wheel 新安装到本轮独立 target**，使用既有 native/reference CPU 环境各跑一次正式 2942 中未改的 `review_snapshot.py`。安全 bootstrap 仅选定安装与探针路径；`python -I` 忽略源码 PYTHONPATH，逐一核对实际导入的七个 ToolAlign 模块均来自该 target 且与 wheel 成员 hash 相同，安装源码/资源共 39 份匹配，源码导入为 0。两 engine 均 exit 0，完整 result 与修复后源路径 result 相同，声明/实际 backend 相同，`!` 均 `[0]`；两个安装重复仍是同一个 F1 场景。native/reference 日志 SHA 为 `3c3894fb4eaf1b4f202b303c9db71f40af3e13fa746b2f3f153ea92330e07aaf` / `f81b0cfbdec7f679eb86dadbc2f97b40ca03f52462acf8ec97d5e5fe12f3b034`。此真实可选 engine 检查与六个默认依赖的纯接口安装分开，真实 AutoTokenizer/返回值/identity 均未替换。

安装来源、未改原 review、当前包载荷及原 full-reference 的只读汇总关联验证 SHA `35844c96b4edc34e6ead742deefecf32465e340b0c53c4066b3cb9f185f489ba`。证据明确构建/纯安装为 b4dc1cf、真实可选引擎的默认 wheel 新安装与反例为 6c82d29；最终仅增加本轮证据文件，载荷继续保持。

**保全和复现。** 范围检查逐字节核对原候选除唯一被修 loader 外的 **234 个公共文件**，随后合入的 **12 份原 R1 公共文件**，以及原 v1 的 **620 个私有制品、37 组原命令元数据与日志**；原失败均未覆盖。原两次构建各 18 个制品实际通过原 manifest 校验，canonical hash 仍为 `87f86783706424553a5c889d51cbbadc33c3c4218e44b3f3ee95058a718b1756`。100 来源/114 决策人审包和填写副本 SHA 保持；CSV 仍为 `eee6b377c78eeff7ab82d65dbc0dcf6934013f546e2439a24309b8b847220efb`，没有填写判断。同步前范围结果 SHA `f6ed0297268d475dd70154b8f97dffa4c8368038d641c582add6b3e3679e8660`；合并后范围结果 SHA `5a90803e2c1adb72d4dac15a9c5a679edafa56601e86224b2748891ea75e3cf2`，新增12份原review也逐字节核对。

证据索引列出其生成前各次本轮实际 argv、环境设置、时间、退出码、完整 log hash、私有命令元数据 hash 和源/结果文件 hash；最终内容扫描、提交/推送/远端读回和完整封存另由本轮私有 completion 及原生交接绑定，避免自引用 hash；本机路径用别名，原文只在私有材料中。复现需已授权的三文件来源和既有两个固定 CPU 环境，参考命令为 `PYTHONPATH=src TOOLALIGN_TOKENIZER_DIR=SOURCE TOOLALIGN_SNAPSHOT_ENGINE=transformers USE_TORCH=0 USE_TF=0 USE_FLAX=0 HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 REFERENCE_PYTHON -m pytest -q tests/model_io/test_snapshot.py`；SOURCE / REFERENCE_PYTHON 是明确占位符，不是额外下载请求。主反例按封存 SHA 对应脚本和证据中的真实参数复现。

本轮新私有目录与命令日志在首个范围检查时共 278972044 字节；合并后范围检查实测为 302560033 字节，后续证据文件与日志计入最终私有封存及 2 GiB 复核，未把旧 R1 环境计为 D1 新建环境。临时来源副本/快照由小型场景限定并由各自运行者清理，未把未仪表化的累计临时写入报告为精确测量。新 tokenizer 环境、下载、权重、模型/GPU、云费用、公开上传、来源重建、8,228 行重跑均为 0。正式格式验收、G-DATA、训练选择、P04/P05/P06、模型质量和部署均未完成；后续由 S0 独立分发，不由本修复自行推进。
