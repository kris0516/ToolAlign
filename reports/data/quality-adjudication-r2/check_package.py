...
def installed(args):
    ...
    from toolalign.data import quality_adjudication as revision
    from toolalign.data import quality_adjudication_materials as materials
    ...
    comparison = materials.compare(**expected["paths"], revision=expected["revision"], frozen=expected["frozen"], reference=expected["reference"], native=expected["native"])
    ...
    result = {"status": "PASS", "actual_Qwen_encoding": "NOT_RUN_IN_DEFAULT_INSTALL",
        "frozen_material_comparison": comparison,
...
def archives_and_install(args):
    ...
    bootstrap = "import sys; sys.path[:0]=[sys.argv.pop(1),sys.argv.pop(1)]; from toolalign.data.quality_adjudication import main; raise SystemExit(main())"
    ...
    run("installed-build-adjudicated-revision", base + ["build", *flags, "--output", str(out / "installed-revision")])
    run("installed-verify-adjudicated-revision", base + ["verify", *flags, "--output", str(out / "installed-revision")])
...
def main():
    ...
    for key in ("root", "archives", "output", "runtime", "inputs", "revision", "frozen", "reference", "native"):
