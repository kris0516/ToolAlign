...
SDIST_SHA = "b8c29ecad579ec43a06b31b9d6106d669dc74c0c50c13d785c861cff46062a17"
...
def archives(args):
    ...
    assert len(expected_source) == 121 and len(expected_package) == 58
    sdist = args.t1_private / "archives-final/default/toolalign-0.0.1.tar.gz"
    assert sha(sdist.read_bytes()) == SDIST_SHA and sdist.stat().st_size == 257973
    tar_contents = {}
    with tarfile.open(sdist) as archive:
        members = archive.getmembers()
        assert len(members) == len({item.name for item in members}) == 122
        for item in members:
    ...
    results = {"sdist": {"path": str(sdist), "sha256": SDIST_SHA, "size_bytes": 257973,
    ...
    for label in ("default", "rebuilt"):
        wheel = args.t1_private / ("archives-final/" + label + "/toolalign-0.0.1-py3-none-any.whl")
        assert sha(wheel.read_bytes()) == WHEEL_SHA and wheel.stat().st_size == 140688
        with zipfile.ZipFile(wheel) as archive:
            ...
            results[label + "_wheel"] = {"path": str(wheel), "sha256": WHEEL_SHA, "size_bytes": 140688,
    ...
    result = {"status": "PASS", "candidate": CANDIDATE, "archives": results,
              "sdist_git_files": 121, "package_files": 58, "new_builds": 0, "new_installs": 0,
              "direct_source_wheel": "NOT_RUN", "existing_archives_parsed": 3}
    save(args.output, result)
    print(json.dumps({"status": "PASS", "archives": 3, "sdist_git_files": 121, "package_files": 58}))
...
def installed(args):
    ...
    names = ("__init__", "__main__", "config", "data", "collator", "plan", "validation", "mlx_adapter", "native_toy")
    for name in names:
    ...
    optional = {"mlx", "mlx_lm", "torch", "transformers", "tokenizers", "tensorflow", "flax", "jax",
                "datasets", "mlx_tune", "mlx_lm_lora", "numpy"}
    ...
    result = {"status": "PASS", "candidate": CANDIDATE, "wheel_sha256": WHEEL_SHA,
              ...
              "new_sft_modules": 9, "package_files": 58, "consumer": consumer_identity(),
              "optional_modules_loaded": [], "source_cwd_imports": 0,
              "isolated": True, "no_site": True, "no_bytecode": True}
    save(args.output, result)
    print(json.dumps({"status": "PASS", "new_sft_modules": 9, "loaded_toolalign_modules": len(origins)}))
