...
from audit_revision_evidence import CANDIDATE, archive_check, audit_demo, blob, package_expectations
...
def main():
    ...
    historical = json.loads((PRIVATE / "historical-binding.json").read_text())["archives"]
    ...
    for name, path in (
        ...
        reference = historical["final/" + name]
    ...
    code = """
import hashlib,importlib,importlib.metadata,importlib.util,json,sys
from pathlib import Path
...
for name in names:
    ...
    module_hashes[name] = hashlib.sha256(Path(module.__file__).read_bytes()).hexdigest()
...
print(json.dumps({'python_version':sys.version.split()[0], 'isolated_flag':sys.flags.isolated,
                  'p03_modules_inside_installed_prefix':names,'model_packages_available':[], 'module_sha256':module_hashes,
...
"""
    ...
    for name, digest in module_evidence["module_sha256"].items():
        relative = "src/" + name.replace(".", "/")
        relative += (
            "/__init__.py" if name in ("toolalign.tools", "toolalign.evaluation.oracles") else ".py"
        )
        assert digest == hashlib.sha256(blob(CANDIDATE, relative)).hexdigest()
    probe_output = run(
        "installed-revision-edges",
        [
            python,
            "-I",
            ROOT / "reports/review/P03-r2/revision_probe.py",
            "--installed",
            "--output",
            workspace / "revision-edges",
        ],
    )
    edge_summary = json.loads(probe_output.strip().splitlines()[-1])
    assert edge_summary["checks"] == edge_summary["passed"] == 31
    assert edge_summary["failed"] == 0 and edge_summary["installed"]
    edge_result = workspace / "revision-edges/results.json"
    record = {
        "candidate": CANDIDATE,
        "archives": archives,
        "commands": records,
        ...
        "installed_revision_summary": edge_summary,
        "installed_revision_result_sha256": hashlib.sha256(edge_result.read_bytes()).hexdigest(),
