...
S0 将每个排除规则的 ASCII 字母展开为大小写字符类，覆盖相同类别的任意大小写组合，并保留 `.env.example` 公开例外。未改变实际 Git/系统配置、依赖或 ML 代码。公共回归在临时合成仓库设置该 Git 条件，241 个私有探针先由 `git check-ignore` 核验（另有一个未列名根文件由 only-include 排除）；大小写变体使用独立父目录，避免在 Mac 上互相覆盖。另有 18 个正常公开对照，包括三种大小写 `.env.example`；实际 sdist、直接 wheel、重建 wheel 均检查内容，冻结 schema 保留。
